...
from .helper_classes import MLP, StagnationEarlyStopping
...
def get_standard_trainer(logger, max_epochs: int, accelerator: str = 'auto',
                         checkpoint_callback: bool = True, **kwargs) -> pl.Trainer:
    """
    Build a pl.Trainer with:
      - StagnationEarlyStopping (patience=10, 1% threshold)
      - LearningRateMonitor
    ...
    callbacks = [
        StagnationEarlyStopping(monitor='val_loss', patience=25, min_delta=0.001, mode='min'),
...
def load_narx_dataset_with_metadata(dataset_path: Union[str, Path]):
    ...
    with h5py.File(dataset_path, 'r') as f:
        for key in f.keys():
            if key != 'metadata':
                data[key] = torch.tensor(f[key][:], dtype=torch.float64)
...
def recursive_narx_rollout(
    ...
    with torch.no_grad():
        for k in range(max_steps):
            ...
            pred_scaled = model.predict(x_tensor).squeeze(0).cpu().numpy().astype(np.float64)
